...
def index(request):
    ...
    if country and year:
        ...
        if month:
            # Ensure month is two digits and remove any leading/trailing spaces
            month = str(month).strip().zfill(2)
            api_url += f"&month={month}"
            print("Month parameter:", month)  # Debug line
        if date:
            api_url += f"&date={date}"
        ...
        if date:
            try:
                # Convert the input date to the same format as API date
                formatted_date = datetime.strptime(date, '%Y-%m-%d').strftime('%Y-%m-%d')
                holidays = [holiday for holiday in holidays if formatted_date == holiday['date']['iso'].split('T')[0]]
                print(f"Filtering by date: {formatted_date}")
            except ValueError:
